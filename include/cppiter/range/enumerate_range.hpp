...
template<typename R>
class enumerate_range : public util::range_facade<iter::enumerate_iterator<util::range_iterator_t<R>>> {
...
public:
    enumerate_range(RngIt begin, RngIt end, typename R::difference_type index) :
        util::range_facade<EnumIt>{ EnumIt{ begin, index }, EnumIt{ end, index } }
    {}
};
