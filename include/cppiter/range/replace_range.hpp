...
class replace_range
  : public util::range_facade<
      iter::replace_iterator<util::range_iterator_t<R>, P>> {
...
public:
  replace_range(
    ...
    RngIt end,
    P pred,
    const iter::util::value_t<RngIt>& newVal
  )
    : util::range_facade<ReplaceIt>{ ReplaceIt{ begin, pred, newVal },
                                     ReplaceIt{ end, pred, newVal } } {
  }
};
